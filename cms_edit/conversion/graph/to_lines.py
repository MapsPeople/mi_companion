...
def osm_xml_to_network(osm_xml: str) -> MultiDiGraph:
    osm_cache_path = ensure_existence(PROJECT_APP_PATH.site_cache) / "from_cms.xml"
...
def network_to_lines(
    graph: MultiDiGraph,
) -> Tuple[List[shapely.LineString], List[Dict]]:
    if False:
        gpkg_cache_path = (
    ...
    return ((*zip(*line_strings),), (*zip(*points),))


def osm_xml_to_lines(osm_xml: str) -> tuple[tuple[Any, Any], tuple[Any, Any]]:
    network = osm_xml_to_network(osm_xml)

    (lines, lines_meta_data), (points, points_meta_data) = network_to_lines(network)
