...
def add_solution_group_contents(
    ...
    solution_group = add_solution_group_if_missing(
        layer_tree_root, mi_hierarchy_group_name, solution
    )

    add_solution_data_layers(qgis_instance_handle, solution, solution_group)

    if ADD_SOLUTION_CONFIG_LAYER:
        add_solution_config_layers(qgis_instance_handle, solution, solution_group)
    ...
    available_location_type_dropdown_widget, location_type_ref_layer = (
        add_location_type_layer_ss(qgis_instance_handle, solution, solution_group)
    )
...
def add_location_type_layer_ss(qgis_instance_handle, solution, solution_group):
    ...
    return available_location_type_dropdown_widget, location_type_ref_layer


def add_solution_group_if_missing(layer_tree_root, mi_hierarchy_group_name, solution):
...
def add_solution_data_layers(qgis_instance_handle, solution, solution_group):
    ...
    return found_solution_data


def add_solution_config_layers(qgis_instance_handle, solution, solution_group):

    found_solution_config = None

    for c in solution_group.children():
        if SOLUTION_CONFIG_DESCRIPTOR in c.name():
            found_solution_config = c
    if found_solution_config is not None:
        reply = QtWidgets.QMessageBox.question(
            None,
            f"Solution config layer for ({found_solution_config.name()}) was already found",
            f"Solution config ({found_solution_config.name()}) will be reloaded with the most recent data from th MapsIndoors database!\n"
            f"Accept?",
        )
        ...
        if reply == QtWidgets.QMessageBox.Yes:
            ...
            ...
    if found_solution_config is None:
        sc = extract_columns_from_solution_config(solution)

        solution_config_layers = add_no_geom_layer(
            qgis_instance_handle=qgis_instance_handle,
            name=SOLUTION_CONFIG_DESCRIPTOR,
            group=solution_group,
            columns=[sc],
            visible=False,
        )
