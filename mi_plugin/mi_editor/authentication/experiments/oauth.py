...
from mi_plugin.mi_editor.authentication.experiments.auth_mode import (
    GCLOUD_COMMAND,
    in_colab_shell,
    in_jupyter_shell,
    localhost_is_viable,
...
from mi_plugin.mi_editor.authentication.experiments.utilities import (
    _open_new_browser,
)
...
MODE_URL = f"{AUTH_PAGE_URL}/mode"
FETCH_URL = f"{AUTH_PAGE_URL}/fetch"
AUTH_URL_TEMPLATE = (
    AUTH_PAGE_URL
    + "?scopes={scopes}&request_id={request_id}&tc={token_challenge}&cc={client_challenge}"
)
    ...
    try:
        response = (
            urllib.request.urlopen(
                TOKEN_URI, urllib.parse.urlencode(request_args).encode()
            )
            .read()
            .decode()
        )
        ...
        raise Exception(f"Problem requesting tokens. Please try again.  {e} {e.read()}")
...
def write_private_json(json_path: str, info_dict: Dict[str, Any]) -> None:
    ...
    if os.path.exists(json_path):
        ...
        os.remove(json_path)
    with os.fdopen(os.open(json_path, os.O_WRONLY | os.O_CREAT, 0o600), "w") as f:
...
def _load_gcloud_credentials(
    ...
    if run_gcloud_legacy:
        ...
        force_quiet = quiet is None and not localhost_is_viable()
    ...
    try:
        ...
        raise Exception(
            "gcloud command not found. " + tip
        ) from e  # pylint:disable=broad-exception-raised
    except subprocess.CalledProcessError as e:
        tip = (
            "Please check for any errors above.\n*Possible fixes:"
    ...
    class Handler(http.server.BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # pylint: disable=invalid-name
            ...
            self.wfile.write(
                ...
                b"  \xe2\x9a\x99\xef\xb8\x8f  \xf0\x9f\x8c\x8e "
            )  # Earth emoji
...
def authenticate(
    ...
    if not auth_mode:
        if in_colab_shell():
            ...
            auth_mode = "notebook"
        elif localhost_is_viable() and no_gcloud():
            auth_mode = "localhost"
        else:
            auth_mode = "gcloud"
    ...
    if auth_mode == "appdefault":
        print(
            "appdefault no longer necessary: ee.Initialize() always checks ADC",
            file=sys.stderr,
        )
        return
...
class Flow:
    ...
    def __init__(
        ...
        if auth_mode == "localhost":
            ...
            self.auth_url = get_authorization_url(
                pkce["code_challenge"], self.scopes, self.server.url
            )
    ...
    def display_instructions(self, quiet: Optional[bool] = None) -> bool:
        ...
        if quiet:
            _display_auth_instructions_for_noninteractive(
                self.auth_url, self.code_verifier
            )
            return True
